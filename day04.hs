...
test1 = part1 testCase == testAnswer -- True
...
combineLines ("" : [])             = []
combineLines (l  : [])             = l : []
combineLines (l  : "" : rest)      = l : combineLines rest
combineLines (l  : r  : rest)      = combineLines $ (l ++ " " ++ r) : rest

parse :: String -> Passport
parse t = M.fromList $ map sep pairs
    where
      pairs     = words t
      toWords w = replace ':' ' ' w
      sep w     = ( head . words $ toWords w
                  , last . words $ toWords w
                  )
...
replace r c (s:ss)
  | ss == []  = s : []
  | s == r    = c : (replace r c ss)
  | s /= r    = s : (replace r c ss)
  | otherwise = s : ss

part1 :: [String] -> Int
part1 lines = length $ filter valid passports
  where
    passports = map parse $ combineLines lines
    valid p   = (M.size p == 7 && M.notMember "cid" p) || M.size p == 8

main :: IO ()
main = do
  contents <- lines <$> readFile "day04.in"
  print "Part 1:"

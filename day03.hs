...
In the above example, these slopes would find 2, 7, 3, 4, and 2 tree(slope) 
...
testAnswer  = 7
testAnswer2 = 336
test1 = numTrees testPatterns (Slope 3 1) == testAnswer -- True
test2 = (product $ map (numTrees testPatterns) testSlopes) == testAnswer2 -- True
...
-- Infinite mountain generator
mountain :: [[Char]] -> [[Char]]
mountain patterns = map cycle patterns

-- List of chars while sledding
encounters :: [[Char]] -> Slope
           -> [Char]
encounters m (Slope x y) = [head $ drop d a | (d, a) <- zip [x, x*2..] (altitudes m y)]
    where
        altitudes m y = drop 1 [altitude | (i, altitude) <- zip [0..] m
                                         , i `mod` y == 0
                                         ]

hits ps slope = encounters (mountain ps) slope
numTrees ps slope = length $ filter isTree (hits ps slope)

main :: IO ()
main = do
    patterns <- lines <$> readFile "day03.in"
    print "Part 1:"
    print $ numTrees patterns (Slope 3 1)
    print "Part 2:"
    print $ product (map (numTrees patterns) testSlopes)
